        ...
        when (msg) {
            "minhasplayers" -> {
                MessageUtils.sendMessage(
                    sender,
                    Translator.translatable(("normal-message.info.min-players-HAS"), someInt.toString())
                )
            ...
            "maxhasplayers" -> {
                MessageUtils.sendMessage(
                    sender,
                    Translator.translatable(("normal-message.info.max-players-HAS"), someInt.toString())
                )
            ...
            "maxhasseekers" -> {
                MessageUtils.sendMessage(
                    sender,
                    Translator.translatable(("normal-message.info.max-seekers"), someInt.toString())
                )
            ...
            "hashints" -> {
                MessageUtils.sendMessage(
                    sender,
                    Translator.translatable(("normal-message.info.hints-HAS"), someInt.toString())
                )
            ...
            "small-slot" -> {
                if (!(someInt >= 1 && someInt <= 9)) {
                    MessageUtils.sendMessage(
                        sender,
                        Translator.translatable(("warning-message.invalid-number"))
                    )
                    return@anyExecutor
                }

                MessageUtils.sendMessage(
                    sender, Translator.translatable(("normal-message.info.small-slot"), someInt.toString())
                )
                ...
                sender,
                Translator.translatable(("warning-message.invalid-config-use"))
            )
        }
